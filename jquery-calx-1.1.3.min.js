...
toNum:function(chr){var base="ABCDEFGHIJKLMNOPQRSTUVWXYZ",i,j,result=0;for(i=0,j=chr.length-1;i<chr.length;i+=1,j-=1)result+=Math.pow(base.length,j)*(base.indexOf(chr[i])+1);return result},toChr:function(num){var s="";num=num-1;while(num>=0){s=String.fromCharCode(num%26+97)+s;num=Math.floor(num/26)-1}return s.toUpperCase()},translateCell:function($cell,$type){var num=$cell.match(/\d+$/);var alpha=$cell.replace(num,"");var $return={row:parseInt(num[0]),col:utility.toNum(alpha)};if($type=="string")return $return.col+
"."+$return.row;else return $return},iterateCell:function(a,b,callback){var $return={index:[],value:[]};var $start=(""+a).split(".");var $stop=(""+b).split(".");$start[1]=$start[1].substring(0,$start[1].length-1);$stop[1]=$stop[1].substring(0,$stop[1].length-1);for(var ax=0;ax<$start.length;ax++){$start[ax]=parseInt($start[ax]);$stop[ax]=parseInt($stop[ax])}var $c_start=$start[0]<$stop[0]?$start[0]:$stop[0];var $c_stop=$start[0]>$stop[0]?$start[0]:$stop[0];var $r_start=$start[1]<$stop[1]?$start[1]:
$stop[1];var $r_stop=$start[1]>$stop[1]?$start[1]:$stop[1];for(col=$c_start;col<=$c_stop;col++)for(row=$r_start;row<=$r_stop;row++){var $cellIndex=utility.toChr(col)+row;var $cellValue=calx.matrix[formula.key].value[$cellIndex];$cellValue=$cellValue?parseFloat($cellValue):0;$return.index.push($cellIndex);$return.value.push($cellValue)}return callback.apply(callback,[$return])}};var defaults={autocalculate:true,trigger:"blur",format:"0[.]00",readonly:true,language:"en",zeroformat:null};var form_element=
...
b,function($cell){var $result=0;for(var i=0;i<$cell.value.length;i++)$result+=$cell.value[i];return $result/$cell.value.length})}};formula.financial={};formula.math={};formula.statistic={};formula.logic={};formula.general={};var matrix=function($key){this.key=$key;this.lang="en";this.zeroformat=null;this.data={};this.value={}};matrix.prototype.update=function($apply){var $dataKey;if(typeof $apply=="undefined")$apply=false;for($dataKey in this.data)if(this.data[$dataKey].dependency.length==0)this.data[$dataKey].updated=
true;else this.data[$dataKey].updated=false;for($dataKey in this.data)if(typeof this.data[$dataKey].formula!="undefined")this.calculate($dataKey,$apply)};matrix.prototype.calculate=function($key,$apply){if(!this.data[$key].updated){if(this.data[$key].dependency.length!=0){var $dkey;for($dkey=0;$dkey<this.data[$key].dependency.length;$dkey++){var $dval=this.data[$key].dependency[$dkey];if(typeof this.data[$dval]=="object")if(!this.data[$dval].updated)this.calculate($dval)}}if(typeof this.data[$key].formula!=
"undefined"){var $replaceVal={};var $stringVal="";var $k;for($k in this.data[$key].dependency){var $v=this.data[$key].dependency[$k];$replaceVal["$"+$v]=this.value[$v];$stringVal+=this.value[$v]}if(this.data[$key].formula.trim()!=""){var $equation="";var $regex="("+formula.member.join("|")+")\\(([^(^)]*)\\)";var $formula_regex=new RegExp($regex,"g");$equation=this.data[$key].formula.replace($formula_regex,function($range){$range=$range.replace(/\$\w+/g,function($key){$key=$key.replace("$","");$key=
utility.translateCell($key,"string");return $key+"1"});return $range});$equation=$equation.replace(/\$\w+/g,function($key){return $replaceVal[$key]||"0"});if($equation.indexOf("$")<0){formula.key=this.key;var $result=utility.parser.parse($equation);this.data[$key].value=isNaN($result)?0:$result;this.value[$key]=isNaN($result)?"":this.data[$key].value}}}}this.data[$key].updated=true;if($apply)this.apply($key)};matrix.prototype.apply=function($key){utility.formatter.language(this.lang);if(typeof $key==
"undefined")$.each(this.value,function($index,$val){var $key=$index.replace(/\$/g,"");var $el=$("#"+$key);if(form_element.indexOf($el.prop("tagName").toLowerCase())>-1)$el.val(utility.formatter(this.value[$key]).format(this.data[$key].format));else $el.html(utility.formatter(this.value[$key]).format(this.data[$key].format))});else{var $el=$("#"+$key);if(form_element.indexOf($el.prop("tagName").toLowerCase())>-1)$el.val(utility.formatter(this.value[$key]).format(this.data[$key].format));else $el.html(utility.formatter(this.value[$key]).format(this.data[$key].format))}};
