(function($) {
    ...
    var formatter = {
         ...
         */
		_currency: function(number, decimals, decimalSeparator, thousandSeparator) {
		...
		getNumber: function(formattedNumber, options) {
			reNum = new RegExp(this._escapeRegex(options.thousandSeparator), 'g');
			reDec = new RegExp(this._escapeRegex(options.decimalSeparator), 'g');
			reSuf = new RegExp(this._escapeRegex(options.suffix), 'g');
			rePref = new RegExp(this._escapeRegex(options.prefix), 'g');
			$trimmed = formattedNumber.replace(/\s/g, '');
			$number = $trimmed.replace(reSuf, ''); //remove suffix
			$number = $number.replace(rePref, ''); //remove prefix
			...
			if (options.format.toLowerCase() == 'percent') {
				return $number / 100;
			} else {
				return $number;
			}
		...
		formatNumber: function(number, options) {
			...
			case 'percent':
				$val = this._percent(number, options.decimal);
				break;
			case 'currency':
				$val = this._currency(parseFloat(number), options.decimal, options.decimalSeparator, options.thousandSeparator);
				break;
			case 'decimal':
			default:
				$val = this._decimal(number, options.decimal);
				break;
			}
			return (!$val) ? number : options.prefix + $val + options.suffix;
		},
    ...
    var matrix    ={
        update  : function(){
            //reset all updated status to false
            $.each(matrix.data,function($key , $dataObj){
                if(matrix.data[$key].dependency.length == 0){
                    matrix.data[$key].updated = true; 
                }else{
                    matrix.data[$key].updated = false; 
                }
            });
            ...
            if(matrix.data[$key].updated==false){
                ...
                if(typeof(matrix.data[$key].formula)!='undefined'){
                    ...
                    if($equation.indexOf('$') < 0){
                        matrix.data[$key].result = parser.parse($equation);
                        console.log('equation result: '+matrix.data[$key].result);
                        matrix.value['$'+$key] = matrix.data[$key].result.toString();
                    }
                }
                
                $('#'+$key).val(matrix.value['$'+$key]);
                matrix.data[$key].updated=true;
                //console.log($dataObj);
            }else{
                console.log('processing '+$key+' : skipped');
            }
        },
        data    : {},
        value   : {}
    ...
    $.fn.calx = function(options){
        return this.each(function(){
            ...
            $form.find('input[type=text]').each(function(){
                ...
                $format     = formatter.parseFormat($format);
                $id         = $input.attr('id');
                ...
                matrix.value['$'+$id]=$value.toString();
                matrix.data[$id] = {
                    'updated'   : false,
                    'value'     : $value,
                    'id'        : $id,
            ...
            $form.find('input:not([readonly])').bind('blur',function(){
                $input      = $(this);
                $id         = $input.attr('id');
                $value      = parseFloat($input.val());
                $value      = isNaN($value) ? 0 : $value;
