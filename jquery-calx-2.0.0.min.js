...
if(!d.isValid()||!f.isValid())return"#VALUE!";if(-1===[0,1,2,3,4].indexOf(c))return"#NUM!";if(d===f)return 0;0<d.diff(f)&&(f=h(new Date(a)),d=h(new Date(b)));var k=d.year(),e=d.month(),r=d.date(),l=f.year();a=f.month();b=f.date();switch(c){case 0:return 31===r&&31===b?b=r=30:31===r?r=30:30===r&&31===b?b=30:1===e&&1===a&&d.daysInMonth()===r&&f.daysInMonth()===b?b=r=30:1===e&&d.daysInMonth()===r&&(r=30),(b+30*a+360*l-(r+30*e+360*k))/360;case 1:c=365;if(k===l||k+1===l&&(e>a||e===a&&r>=b)){if(k===l&&
h([k]).isLeapYear())c=366;else if(k=d,l=f,e=h(new Date(k.year(),2,1)),h([k.year()]).isLeapYear()&&0>k.diff(e)&&0<=l.diff(e)?k=!0:(e=h(new Date(l.year(),2,1)),k=h([l.year()]).isLeapYear()&&0<=l.diff(e)&&0>k.diff(e)?!0:!1),k||1===a&&29===b)c=366;return f.diff(d,"days")/c}a=l-k+1;a=h(new Date(l+1,0,1)).diff(h(new Date(k,0,1)),"days")/a;return f.diff(d,"days")/a;case 2:return f.diff(d,"days")/360;case 3:return f.diff(d,"days")/365;case 4:return 31===r&&(r=30),31===b&&(b=30),(b+30*a+360*l-(r+30*e+360*
...
"'").replace("&#34;",'"'):!1;c&&""!=p.trim(c)||(c=this.sheet.config.defaultFormat);this.formula=b;this.format=c;this.address=a;this.setValue(d)};u.fx.calculate=function(a){a="undefined"==typeof a?!0:a;this.sheet.config.autoCalculate&&a&&"function"==typeof this.sheet.config.onBeforeCalculate&&this.sheet.config.onBeforeCalculate.apply(this.sheet);s.isCalculating=!0;this.evaluateFormula();for(var b in this.dependant)this.dependant[b].processDependant();for(b in this.sheet.dependant)this.sheet.dependant[b].calculate(!1);
s.isCalculating=!1;if(this.sheet.hasRelatedSheet())for(b in this.sheet.cells)this.sheet.cells[b].hasRemoteDependency()&&(this.sheet.cells[b].evaluateFormula(),this.sheet.cells[b].processDependant(),this.sheet.cells[b].renderComputedValue());this.sheet.config.autoCalculate&&a&&"function"==typeof this.sheet.config.onAfterCalculate&&this.sheet.config.onAfterCalculate.apply(this.sheet);this.sheet.config.autoCalculate&&a&&"function"==typeof this.sheet.config.onBeforeRender&&this.sheet.config.onBeforeRender.apply(this.sheet);
this.renderComputedValue();this.sheet.config.autoCalculate&&a&&"function"==typeof this.sheet.config.onAfterRender&&this.sheet.config.onAfterRender.apply(this.sheet);return this};u.fx.buildDependency=function(){var a={remoteCellRange:/\#[A-Za-z0-9_]+\s*!\s*[A-Za-z]+[0-9]+\s*:\s*[A-Za-z]+[0-9]+/g,remoteCell:/\#[A-Za-z0-9_]+\s*!\s*[A-Za-z]+[0-9]+/g,cellRange:/[A-Za-z]+[0-9]+\s*:\s*[A-Za-z]+[0-9]+/g,cell:/[A-Z]+[0-9]+/g},b=this.formula,c="#"+this.sheet.el.attr("id"),d=this.address,f,e,g,h,l,m,n;for(e in this.dependencies)-1===
e.indexOf("#")?this.dependencies[e].removeDependant(d):this.dependencies[e].removeDependant(c+"!"+d),delete this.dependencies[e];if(b)for(e in a)if(l=b.match(a[e]),b=b.replace(a[e],""),null!==l)switch(e){case "remoteCellRange":for(d=0;d<l.length;d++)for(g in f=l[d].split("!"),m=p.trim(f[0]),h=f[1].split(":"),f=p.trim(h[0]),h=p.trim(h[1]),f=this.sheet.getRemoteCellRange(m,f,h),n=p(m).attr("data-calx-identifier"),"undefined"!=typeof n&&"undefined"!=typeof s.sheetRegistry[n]&&(s.sheetRegistry[n].registerDependant(this.sheet),
this.sheet.registerDependency(s.sheetRegistry[n])),f)h=m+"!"+g,"undefined"==typeof this.dependencies[h]&&!1!==f[g]&&(this.hasRemoteDependency(!0),this.dependencies[h]=f[g],f[g].registerDependant(c+"!"+this.getAddress(),this));break;case "remoteCell":for(d=0;d<l.length;d++)f=l[d].split("!"),m=p.trim(f[0]),h=p.trim(f[1]),f=this.sheet.getRemoteCell(m,h),n=p(m).attr("data-calx-identifier"),"undefined"!=typeof n&&"undefined"!=typeof s.sheetRegistry[n]&&(s.sheetRegistry[n].registerDependant(this.sheet),
this.sheet.registerDependency(s.sheetRegistry[n])),h=m+"!"+h,"undefined"==typeof this.dependencies[h]&&!1!==f&&(this.hasRemoteDependency(!0),this.dependencies[h]=f,f.registerDependant(c+"!"+this.getAddress(),this));break;case "cellRange":for(d=0;d<l.length;d++)for(g in h=l[d].split(":"),f=p.trim(h[0]),h=p.trim(h[1]),f=this.sheet.getCellRange(f,h),f)"undefined"==typeof this.dependencies[g]&&!1!==f[g]&&(this.dependencies[g]=f[g],f[g].registerDependant(this.getAddress(),this));break;case "cell":for(d=
0;d<l.length;d++)h=l[d],f=this.sheet.getCell(h),"undefined"==typeof this.dependencies[h]&&!1!==f&&(this.dependencies[h]=f,f.registerDependant(this.getAddress(),this))}return this};u.fx.removeDependency=function(a){"undefined"!=typeof this.dependencies[a]&&delete this.dependencies[a]};u.fx.processDependency=function(){if(0==this.isProcessed()){for(var a in this.dependencies)0==this.dependencies[a].isProcessed()&&this.dependencies[a].processDependency();this.evaluateFormula();this.setProcessed(!0)}};
u.fx.registerDependant=function(a,b){"undefined"==typeof this.dependant[a]&&b&&(this.dependant[a]=b)};u.fx.removeDependant=function(a){"undefined"!=typeof this.dependant[a]&&delete this.dependant[a]};u.fx.processDependant=function(){var a;if(!1===this.isProcessed()||!0===s.isCalculating){this.processDependency();for(var b in this.dependant)if(!this.dependant[b].isProcessed()&&(a=this.dependant[b].processDependant(),!1===a))return a;this.setAffected(!1);this.setProcessed(!0)}else return!1};u.fx.hasRemoteDependency=
