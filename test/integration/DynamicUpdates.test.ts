...
describe('Dynamic Updates', () => {
    describe('Dynamic Cell Creation', () => {
        test('should update formula when new cells are added', () => {
            ...
            sheet.createCell('A6', { formula: '=SUM(A1:A2)' });

            workbook.build();
            workbook.calculate();

            expect(sheet.getCellValue('A6')).toBe(30);

            // Dynamically add new cells that should be included in existing formulas
            // First, update the formula to include a wider range (avoiding circular reference)
            sheet.getCellDirect('A6').formula = '=SUM(A1:A5)';
            ...
            sheet.createCell('A4', { value: 25 });
            sheet.createCell('A5', { value: 30 });

            // Re-enable auto-calculate and rebuild/calculate
            sheet.autoCalculate = true;
            // Need to rebuild after formula change and new cells
            workbook.build();
            workbook.calculate();

            // Should now include all cells
            expect(sheet.getCellValue('A6')).toBe(100); // 10+20+15+25+30
        });
    ...
    describe('Dynamic Column/Row References', () => {
        ...
        test('should handle column reference with formulas in the column', () => {
            ...
            sheet.createCell('B1', { formula: '=SUM(A1:A3)' }); // Use specific range instead
            ...
            // Add more to column A and update formula to include it
            sheet.createCell('A4', { value: 40 });
            sheet.getCellDirect('B1').formula = '=SUM(A1:A4)';
    ...
    describe('Cross-Sheet Dynamic Updates', () => {
        ...
        test('should handle cross-sheet column references', () => {
            ...
            summary.createCell('B1', { formula: '=SUM(Data!A1:A2)' });
            ...
            // Add more cells to Data!A column and update formula
            data.createCell('A3', { value: 30 });
            data.createCell('A4', { value: 40 });
            summary.getCellDirect('B1').formula = '=SUM(Data!A1:A4)';
